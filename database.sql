...
CREATE TABLE IF NOT EXISTS notifications (id INT UNSIGNED AUTO_INCREMENT PRIMARY KEY, recruiter_id INT UNSIGNED NOT NULL, title VARCHAR(255) NOT NULL, message TEXT NOT NULL, image_path VARCHAR(255) NULL, is_read TINYINT(1) NOT NULL DEFAULT 0, created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP, CONSTRAINT fk_notifications_recruiter FOREIGN KEY (recruiter_id) REFERENCES users(id) ON DELETE CASCADE, INDEX idx_recruiter_unread (recruiter_id, is_read)) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;
...
CREATE TABLE IF NOT EXISTS city_rates (id INT UNSIGNED AUTO_INCREMENT PRIMARY KEY, city VARCHAR(120) NOT NULL UNIQUE, reward_per_order INT UNSIGNED NOT NULL DEFAULT 0, reward_auto INT UNSIGNED NOT NULL DEFAULT 0, reward_foot INT UNSIGNED NOT NULL DEFAULT 0, max_earnings_per_courier INT NULL DEFAULT NULL, created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;
...
INSERT INTO city_rates (city, reward_per_order, reward_auto, reward_foot, max_earnings_per_courier) VALUES ('Москва',30,30,30,NULL),('Санкт-Петербург',30,30,30,NULL),('Казань',25,25,25,NULL) ON DUPLICATE KEY UPDATE reward_per_order=reward_per_order;
...
-- Миграция для существующей базы вынесена в migrations/20260616_city_rates_auto_foot_news_images.sql.
